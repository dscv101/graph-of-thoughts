        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ImportFrom):
                    if (
                        node.module
                        and "graph_of_thoughts.language_models" in node.module
                    ):
            ...
            findings.append(
                {"type": "analysis_error", "file": str(file_path), "error": str(e)}
            )
...
class MCPConfigGenerator:
    ...
    TEMPLATES = {
        "claude-desktop": {
            ...
            "default_sampling_params": {"maxTokens": 4096, "temperature": 0.7},
            "retry_config": {
                "max_attempts": 3,
                "base_delay": 1.0,
    ...
    def generate_config(
        self, host_type: str, output_path: str = "mcp_config.json"
    ) -> bool:
        ...
        config = {f'mcp_{host_type.replace("-", "_")}': self.TEMPLATES[host_type]}
    ...
    for file_path, file_findings in by_file.items():
        ...
        for finding in file_findings:
            if finding["type"] == "legacy_import":
                ...
                print(
                    f"   Line {finding['line']}: Legacy instantiation '{finding['class']}'"
                )
                print(f"   💡 {finding['suggestion']}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Migrate from legacy language models to MCP"
    )

    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument(
        "--analyze", metavar="PATH", help="Analyze directory for legacy usage"
    )
    group.add_argument(
        "--generate-config", action="store_true", help="Generate MCP configuration"
    )
    group.add_argument(
        "--test-connection", metavar="CONFIG", help="Test MCP connection"
    )

    parser.add_argument(
        "--host",
        choices=["claude-desktop", "vscode", "remote-http"],
        help="MCP host type for config generation",
    )
    parser.add_argument(
        "--output", default="mcp_config.json", help="Output path for generated config"
    )
    ...
    if args.analyze:
        ...
        if success:
            ...
            print(
                "2. Test connection: python migrate_to_mcp.py --test-connection "
                + args.output
            )
