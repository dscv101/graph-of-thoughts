...
from .mcp_host_plugins import (
    MCPHostPlugin, HostCapabilities, register_host_plugin, get_host_plugin,
    list_available_hosts, get_host_capabilities, create_transport_from_plugin,
    generate_config_template, validate_host_config, discover_available_hosts,
...
class MCPPluginManager:
    ...
    def register_plugin(self, plugin: MCPHostPlugin) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Successfully registered plugin: {plugin.get_display_name()}")
            return True
        except Exception as e:
            self.logger.error(f"Failed to register plugin {plugin.get_display_name()}: {e}")
            return False
    ...
    def create_host_config(self, host_name: str, **overrides) -> Optional[Dict[str, Any]]:
        """
