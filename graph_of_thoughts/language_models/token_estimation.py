...
class TokenEstimator:
    ...
    def estimate_tokens(self, text: str, context: str = "general") -> int:
        ...
        adjusted_tokens = self._apply_context_adjustments(
            base_tokens, analysis, context
        )
    ...
    def _calculate_base_tokens(self, analysis: Dict) -> float:
        ...
        base_tokens += (
            analysis["punctuation_count"] * self.config.punctuation_token_ratio
        )

        # Adjust for numbers (often tokenize differently)
        number_adjustment = analysis["number_count"] * (
            self.config.number_token_ratio - 1
        )
        ...
        if self.config.enable_subword_estimation and analysis["long_words"] > 0:
            subword_adjustment = analysis["long_words"] * (
                self.config.subword_split_ratio - 1
            )
