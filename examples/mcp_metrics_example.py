...
async def demonstrate_metrics_collection():
    ...
    config_path = Path(__file__).parent.parent / "graph_of_thoughts" / "language_models" / "mcp_config_with_metrics_template.json"
    ...
    try:
        ...
        print(f"✅ Initialized MCP client with metrics: {lm.metrics_collector is not None}")
    ...
    async with lm:
        ...
        if cb_status:
            ...
            if cb_status['total_requests'] > 0:
                error_rate = (cb_status['failed_requests'] / cb_status['total_requests']) * 100
