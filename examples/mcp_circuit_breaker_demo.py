...
from graph_of_thoughts.language_models.mcp_circuit_breaker import (
    MCPCircuitBreaker, CircuitBreakerConfig, CircuitBreakerState,
    CircuitBreakerOpenError, create_circuit_breaker_from_config
)
...
class MockFailingService:
    ...
    def set_failure_mode(self, enabled: bool, failure_rate: float = 1.0):
        ...
        print(f"🔧 Service failure mode: {'ON' if enabled else 'OFF'} (rate: {failure_rate})")
...
async def demonstrate_basic_circuit_breaker():
    ...
    print(f"📊 Metrics: {metrics.successful_requests} success, {metrics.failed_requests} failures")
    ...
    metrics = circuit_breaker.get_metrics()
    print(f"📊 Metrics: {metrics.successful_requests} success, {metrics.failed_requests} failures, {metrics.circuit_open_count} opens")
    ...
    print("\n🟡 Phase 4: Recovery Testing (Half-Open)")
    service.set_recovery_after_calls(service.call_count + 2)  # Recover after 2 more calls
...
async def demonstrate_config_based_circuit_breaker():
    ...
    if circuit_breaker:
        ...
        for i in range(4):
            try:
                result = await circuit_breaker.call(service.call_service, f"config_test_{i+1}")
                print(f"  ✅ {result}")
...
async def demonstrate_metrics_monitoring():
    ...
    for i in range(10):
        ...
        if (i + 1) % 3 == 0:
            ...
            print(f"     📊 Success rate: {success_rate:.1f}% ({metrics.successful_requests}/{metrics.total_requests})")
