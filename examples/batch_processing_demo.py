...
class SimpleBatchParser(BatchParser):
    ...
    def parse_generate_answer(self, state: Dict, texts: List[str]) -> List[Dict]:
        ...
        return [{"content": text, "parent": state.get("id", "unknown")} for text in texts]
...
async def demonstrate_batch_generation(lm, prompter, parser, topics):
    ...
    try:
        ...
        logger.info(f"Generated {len(generated_thoughts)} thoughts in {end_time - start_time:.2f} seconds")
...
async def performance_comparison(lm, prompter, parser, topics):
    ...
    if batch_times and sequential_times:
        ...
        logger.info(f"Average sequential processing time: {avg_sequential_time:.3f} seconds")
        logger.info(f"Speedup factor: {speedup:.2f}x")
        
        if speedup > 1:
            logger.info("✅ Batch processing is faster!")
        else:
            logger.info("⚠️  Sequential processing was faster (possibly due to overhead)")
