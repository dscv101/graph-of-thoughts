...
class PerformanceBenchmark:
    ...
    async def benchmark_sequential_processing(self, num_requests: int) -> BenchmarkResult:
    ...
    async def benchmark_batch_processing(
        ...
        logger.info(f"Benchmarking batch processing with {num_requests} requests "
                   f"(max_concurrent={max_concurrent}, batch_size={batch_size})")
        
        # Prepare queries
        queries = [self.test_queries[i % len(self.test_queries)] for i in range(num_requests)]
    ...
    def print_results(self, results: List[BenchmarkResult]):
        ...
        # Header
        print(f"{'Method':<12} {'Requests':<10} {'Time(s)':<8} {'Success':<8} {'Failed':<8} "
              f"{'RPS':<8} {'Latency(s)':<12} {'Error%':<8}")
        print("-" * 80)
        
        # Results
        for result in results:
            print(f"{result.method:<12} {result.total_requests:<10} {result.total_time:<8.2f} "
        ...
        if len(results) >= 2:
            ...
            if sequential and batch and sequential.throughput > 0:
                ...
                latency_improvement = sequential.avg_latency / batch.avg_latency if batch.avg_latency > 0 else 0
                ...
                if throughput_improvement > 1.2:
                    print("✅ Batch processing shows significant performance improvement!")
                elif throughput_improvement > 1.0:
                    print("✅ Batch processing shows modest performance improvement")
                else:
                    print("⚠️  Sequential processing was faster (possibly due to overhead)")
    ...
    async def run_scalability_test(self):
        ...
        for count in request_counts:
            ...
            # Test batch
            batch_result = await self.benchmark_batch_processing(count, max_concurrent=5)
        ...
        print("="*60)
        print(f"{'Requests':<10} {'Sequential RPS':<15} {'Batch RPS':<12} {'Speedup':<10}")
        print("-" * 60)
        ...
        for i in range(0, len(results), 2):
            if i + 1 < len(results):
                ...
                print(f"{seq.total_requests:<10} {seq.throughput:<15.1f} "
