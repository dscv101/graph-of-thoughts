...
class SimpleParser(BatchParser):
    ...
    def parse_generate_answer(self, state: Dict, texts: List[str]) -> List[Dict]:
        ...
        return [
            {"content": text.strip(), "source": state.get("topic", "unknown")}
            for text in texts
            if text.strip()
        ]
...
async def simple_batch_example():
        ...
        async with lm:
            ...
            for topic in topics:
                ...
                for i, thought in enumerate(thoughts):
                    logger.info(
                        f"  {i+1}. {thought.state.get('content', 'No content')}"
                    )
    ...
    try:
        async with lm:
            ...
            responses = await lm.query_batch(queries, max_concurrent=3, batch_size=10)
