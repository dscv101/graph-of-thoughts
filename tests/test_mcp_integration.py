    ...
    async def test_mcp_language_model_initialization(self):
        ...
        try:
            with patch(
                ...
                # Test initialization
                lm = MCPLanguageModel(config_path=config_file, model_name="mock_stdio")
    ...
    async def test_error_handling_connection_failure(self):
        ...
        try:
            with patch(
                ...
                mock_transport = AsyncMock()
                mock_transport.connect.side_effect = MCPConnectionError(
                    "Connection failed"
                )
                mock_create.return_value = mock_transport
    ...
    async def test_error_handling_timeout(self):
        ...
        try:
            with patch(
                ...
                mock_transport = AsyncMock()
                mock_transport.connect.return_value = True
                mock_transport.send_sampling_request.side_effect = MCPTimeoutError(
                    "Request timed out"
                )
                mock_create.return_value = mock_transport
