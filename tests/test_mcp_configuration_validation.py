...
class TestMCPConfigurationValidation(unittest.TestCase):
    ...
    def test_missing_stdio_command(self):
        ...
        self.assertIn(
            "Missing required field for stdio transport: command", str(cm.exception)
        )
    ...
    def test_invalid_include_context(self):
        ...
        self.assertEqual(
            cm.exception.field_path, "default_sampling_params.includeContext"
        )
    ...
    def test_invalid_model_preferences(self):
        ...
        self.assertIn(
            "Model hint at index 0 must be an object with 'name' field",
            str(cm.exception),
        )
    ...
    def test_invalid_cost_tracking(self):
        ...
        self.assertIn(
            "prompt_token_cost must be a non-negative number", str(cm.exception)
        )
    ...
    def test_http_security_validation(self):
        ...
        config["transport"] = {
            ...
            "headers": {"Authorization": "Bearer abc"},  # Short token
        }
    ...
    def test_configuration_template_generation(self):
        ...
        # Test stdio template
        stdio_template = self.validator.generate_configuration_template(
            "stdio", "test_stdio"
        )
        ...
        # Test HTTP template
        http_template = self.validator.generate_configuration_template(
            "http", "test_http"
        )
