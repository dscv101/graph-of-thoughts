...
from graph_of_thoughts.language_models.mcp_transport import (
    create_transport, MCPConnectionError, MCPTimeoutError, MCPServerError
)
...
class MCPIntegrationTestFramework:
    ...
    def __init__(self, args):
        ...
        self.skip_slow = os.environ.get("MCP_SKIP_SLOW_TESTS", "false").lower() == "true"
    ...
    def _load_test_suites(self):
        ...
        self.test_suites.append(TestSuite(
            ...
            config={"timeout": 30, "test_prompts": [
                "Hello, world!",
            ...
            if Path(path).exists():
                try:
                    ...
                    for model_name, model_config in config.items():
                        if model_name.startswith("mcp_"):
                            # Avoid duplicates
                            if not any(name == model_name for name, _ in configurations):
                                configurations.append((model_name, model_config))
                                self.logger.info(f"Discovered configuration: {model_name} from {path}")
                except Exception as e:
    ...
    async def validate_configuration(self, model_name: str, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            for field in required_fields:
                if field not in config:
                    self.logger.error(f"Missing required field '{field}' in {model_name}")
                    return False
            ...
            if transport_type == "stdio":
                if "command" not in transport_config:
                    self.logger.error(f"Missing command for stdio transport in {model_name}")
                    return False
                ...
                self.logger.error(f"Unsupported transport type '{transport_type}' in {model_name}")
                return False
    ...
    async def test_connection_establishment(self, model_name: str, config: Dict[str, Any]) -> TestResult:
        """Test basic connection establishment."""
        start_time = time.time()
        
        try:
            # Create language model
            lm = MCPLanguageModel(config={"test_model": config}, model_name="test_model")
            ...
            async with lm:
                ...
                return TestResult(
                    ...
                    details={"model_name": model_name, "transport_type": config["transport"]["type"]}
                )
    ...
    async def test_simple_text_generation(self, model_name: str, config: Dict[str, Any]) -> TestResult:
        ...
        try:
            ...
            async with lm:
                # Test simple query
                response = await lm.query_async("Hello, this is a test. Please respond.")
                ...
                return TestResult(
                    ...
                    details={
                        ...
                        "response_preview": response[0][:100] + "..." if len(response[0]) > 100 else response[0],
    ...
    async def run_test_suite(self, suite: TestSuite, model_name: str, config: Dict[str, Any]) -> TestSuite:
        ...
        for test_name in suite.tests:
            ...
            try:
                ...
                if test_name == "test_connection_establishment":
                    result = await self.test_connection_establishment(model_name, config)
