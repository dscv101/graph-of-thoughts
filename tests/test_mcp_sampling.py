...
class TestSamplingRequestCreation(unittest.TestCase):
    ...
    def test_create_sampling_request_with_model_preferences(self):
        ...
        request = create_sampling_request(messages, model_preferences=model_preferences)
...
class TestMCPSamplingManager(AsyncTestCase):
    ...
    async def _test_single_request(self):
        ...
        messages = [create_text_message("user", "Test prompt")]
        response = await self.manager.create_message(messages, temperature=0.5)
    ...
    async def _test_batch_with_custom_params(self):
        ...
        await self.manager.create_batch_completions(
            prompts, temperature=0.3, max_tokens=500
        )
    ...
    async def _test_concurrent_limiting(self):
        ...
        self.assertLessEqual(
            max_concurrent, self.config["batch_processing"]["max_concurrent"]
        )
